...
class WebActivityTest {
    ...
    private fun assertStatusBar(expectedLight: Boolean) {
        withWebActivity { activity ->
            ...
            assertEquals(expectedLight, WindowInsetsControllerCompat(activity.window, rootView).isAppearanceLightStatusBars)
            assertEquals(activity.resources.getColor(R.color.ixigosdk_primary_color), activity.window.statusBarColor)
        }
    }
