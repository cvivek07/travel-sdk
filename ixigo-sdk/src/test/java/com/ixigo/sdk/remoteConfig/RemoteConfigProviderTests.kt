...
class FirebaseRemoteConfigProviderTests {
  ...
  fun `test Json value`() {
    ...
    val defaultPerson = Person("johnnie", 25)
    val provider = RemoteConfigProvider(firebaseRemoteConfig)
    val person = provider.get("known", defaultPerson)
    assertEquals(Person("johnnie", 25), person)

    assertEquals(defaultPerson, provider.get("unknown", defaultPerson))
  }
  ...
  fun `test string value`() {
    ...
    val provider = RemoteConfigProvider(firebaseRemoteConfig)
    val value = provider.get("known", defaultValue)
