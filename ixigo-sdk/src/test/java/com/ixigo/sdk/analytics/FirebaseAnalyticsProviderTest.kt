...
class FirebaseAnalyticsProviderTest {
    ...
    private fun withFirebase(block: () -> Unit) {
        mockStatic(FirebaseAnalytics::class.java).use { staticMock ->
            staticMock.`when`<Any>{ FirebaseAnalytics.getInstance(context)}.thenReturn(mockFirebaseAnalytics)
            block()
        }
    }
    ...
    private fun withoutFirebase(block: () -> Unit) {
        mockStatic(FirebaseAnalytics::class.java).use { staticMock ->
            staticMock.`when`<Any>{ FirebaseAnalytics.getInstance(context)}.thenThrow(NoClassDefFoundError())
            block()
        }
    }
}